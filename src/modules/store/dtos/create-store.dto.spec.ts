...
describe('CreateStoreDto', () => {
  it('should pass validation', async () => {
    ...
    const dto = plainToInstance(CreateStoreDto, receivedData);
  ...
  it('should fail validation without name', async () => {
    ...
    expect(stringifiedErrors).toContain('name should not be empty');
  });
  ...
  it('should fail validation without ecommerceUrl', async () => {
    const receivedData = {
      name: 'Store A1',
    };
    const dto = plainToInstance(CreateStoreDto, receivedData);
    ...
    expect(errors.length).not.toBe(0);
    expect(stringifiedErrors).toContain('ecommerceUrl should not be empty');
  });
